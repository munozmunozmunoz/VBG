...
dat<-read.csv('VBG_Data.csv',na.strings='NA', header=T)
...
# Number of bays
N <- length(unique(dat$STUDY_BAY_NO))


# Set plotting margins
par(mfrow=c(1,1),mar=c(1,1,2,5),oma=c(6,4,2,6),mai=c(0.5,0.5,0,0.5))
# Plot with no data
plot(SVL ~ Time, data=dat, pch=16, xlim=c(min(dat$Time), max(dat$Time) ),xaxt="n",yaxt="n",type='n',xlab='',ylab='')

axis(side=1,cex.axis=2, mgp=c(1,1,0),tck= -0.01) 
axis(side=2,cex.axis=2,mgp=c(1,1,0),tck= -0.01)

# Add data points
points(jitter(dat$Time),dat$SVL, cex=2, pch=16,col=dat$Color ,bg=as.numeric(dat$Color), lwd=2)
...
dat$g <- as.numeric(dat$Color)

# ######### nls analysis - use these as starting values for matrix B #####
#  c1 <- array(NA,c(N,3) )
# 
#  for(i in 1:N){
#   vb1 <- nls(length ~ Linf*(1-exp(-k*(age-to))), data=dat, subset=g==i, nls.control(maxiter=5000,minFactor=1e-20), start=list(Linf=400, k=0.03, to=-2))
...
cat("
    ...
    Tau.B[1:3,1:3] ~ dwish(W[,], dat) # Variance-covariance matrix from precision matrix (i.e., 1/tau)
    dat <- 4 # dat set to 1 greater than dimension of Tau.B (i.e., the number of varying parameters + 1)
...
# load data
data <- list(y = dat$SVL, age = dat$Time, g = dat$Color, n = dim(dat)[1],
             J = length(unique(dat$Color)), W=W  )


# Number of parameters
K <- 3
# Number of groups
J <- length(unique(dat$Color))
...
predX <- seq(min(dat$age), max(dat$age), length=100) # fake data to predict
...
z <- seq(min(dat$length),max(dat$length),length=100) # fake y-axis
